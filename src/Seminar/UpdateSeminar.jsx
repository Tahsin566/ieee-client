...
const UpdateSeminar = () => {
    ...
    const { getSeminarById, singleSeminar, updateSeminar, loading } = useSeminar();
    ...
    useEffect(() => {
        if (singleSeminar) {
            setFormData({
                ...
                description: singleSeminar.description || '',
                bannerImage: null,
                speakerName: singleSeminar.speaker?.name || '',
                speakerDesignation: singleSeminar.speaker?.designation || '',
                speakerOrganization: singleSeminar.speaker?.organization || '',
                speakerPhoto: null,
                date: singleSeminar.date ? new Date(singleSeminar.date).toISOString().split('T')[0] : '',
                time: singleSeminar.time || '',
                location: singleSeminar.location || '',
                driveLink: singleSeminar.driveLink || '',
                registrationLink: singleSeminar.registrationLink || ''
            });
            setSeminarType(singleSeminar.type || 'seminar');
            setCategory(singleSeminar.category || 'technical');
            setStatus(singleSeminar.status || 'upcoming');
            setIsFeatured(singleSeminar.isFeatured || false);
            setBannerPreview(singleSeminar.bannerImage);
            setSpeakerPhotoPreview(singleSeminar.speaker?.photo);
        }
    }, [singleSeminar]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.title || !formData.description || !formData.speakerName || 
            !formData.speakerDesignation || !formData.date || !formData.time || !formData.location) {
        ...
        const result = await updateSeminar(id, submitData);
    ...
    if (userLoading || (loading && !singleSeminar)) {
        return (
